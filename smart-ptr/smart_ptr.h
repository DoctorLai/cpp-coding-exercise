...
template <typename T> class SharedPtr
{
  ...
  private:
    ...
    SharedPtr(T* ptr, RefBlockBase* block) noexcept : m_ptr(ptr), m_block(block) {}
  ...
  public:
    ...
    // accept rao ptr
    explicit SharedPtr(T* ptr) : SharedPtr(ptr, std::default_delete<T>()) {}
    ...
    template <
        ...
        typename D,
        typename =
            typename std::enable_if<!std::is_convertible<typename std::decay<D>::type, RefBlockBase*>::value>::type>
    SharedPtr(Y* ptr, D&& deleter) : m_ptr(ptr)
    {
    ...
    // move constructor
    SharedPtr(SharedPtr&& other) noexcept : m_ptr(other.m_ptr), m_block(other.m_block)
    {
        other.m_ptr = nullptr;
    ...
    // --- copy control ---
    SharedPtr(const SharedPtr& other) : m_ptr(other.m_ptr), m_block(other.m_block)
    {
        if (m_block) {
...
template <typename T> class WeakPtr
{
  ...
  public:
    ...
    WeakPtr(const SharedPtr<T>& shared) noexcept : m_block(shared.m_block)
    {
        if (m_block) {
